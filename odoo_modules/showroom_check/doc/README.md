...
3. **Set up the rounds.** Showroom Check → Configuration → Settings:
...
A cleaner opens **Showroom Check**. Each round for today is shown as a card:
...
Turn on developer mode, open the Showroom Check dashboard, and press
...
```
odoo-bin -d <database> -i showroom_check --test-enable \
         --test-tags /showroom_check --stop-after-init
```
